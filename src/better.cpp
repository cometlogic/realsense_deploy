    ...
    while (true) {
        ...
        for (int i = 0; i < depth_image.rows; ++i) {
            for (int j = 0; j < depth_image.cols; ++j) {
                ...
                if (depth_in_meters > depth_clipping_distance[1]) {
                    depth_image.at<uint16_t>(i, j) = depth_clipping_distance[1]; // 超过6米的深度值设为6
                } else if (depth_in_meters < depth_clipping_distance[0]) {
                    depth_image.at<uint16_t>(i, j) = depth_clipping_distance[0]; // 小于0的深度值设为0
                }
            }
        }
